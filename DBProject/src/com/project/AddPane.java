...
public class AddPane extends JPanel implements ActionListener, ItemListener {
	...
	public AddPane() {
		...
		for (i = 0; i < size - 1; i++) {
			...
			add(jp[i]); // jp패널을 JPanel에 묶어서 올린다는 의미!
		}
	...
	@Override
	public void itemStateChanged(ItemEvent e) {// 아이템의 상태를 알아보는 기능
	...
	@Override
	public void actionPerformed(ActionEvent ae) {// 버튼 누르면 저장/ 리셋 해주는 기능
		String ae_type = ae.getActionCommand();
		EmployeeVO evo = null;
		EmployeeDAO edvo = null;

		if (ae_type.equals(okb.getText())) { // 저장
			try {
		...
		} else if (ae_type.equals(rsb.getText())) { // 리셋
			int size = caption.length;
